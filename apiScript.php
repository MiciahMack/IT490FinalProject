...
$compare = "SELECT * FROM queryValidation WHERE ValidatedQuery='$searchVal'";
...
if ($search1->num_rows > 0) {	
	...
	echo "<table><tr><th>Artist</th><th>Title</th><th>Label</th><th>Genre</th><th>Year</th><th>Image</th><th>Country</th></tr>";
              
	while($row = $search2->fetch_assoc()) {
        	echo "<tr><td>". $row["Artist"]. "</td><td>" . $row["Title"]. "</td><td>" . $row["Label"]. "</td><td>" . $row["Genre"]
	...
	if ($pages >= 150) {
		...
		while (($line2 = fgets($myfile3)) !== false) {
				...
				$var = 1;
					...
					continue;
                                ...
                                if ($var == 1) {
                                        $labelVal = $line2;
                                	$track = "INSERT INTO queryResults (Artist, Country, Label, Genre, Year, Image, Title) VALUES('$searchVal', '$countryVal', '$labelVal', '$styleVal', '$yearVal', '$imageVal', '$titleVal')";
		...
		fclose($myfile3);
		$display3 = "SELECT * FROM queryResults WHERE Artist='$searchVal'";
		$search4 = mysqli_query($conn,$display3);

        	echo "<table><tr><th>Artist</th><th>Title</th><th>Label</th><th>Genre</th><th>Year</th><th>Image</th><th>Country</th></tr>";

        	while($row = $search4->fetch_assoc()) {
                	echo "<tr><td>". $row["Artist"]. "</td><td>" . $row["Title"]. "</td><td>" . $row["Label"]. "</td><td>" . $row["Genre"]. "</td><td>" . $row["Year"]."</td><td>" . $row["Image"]. "</td><td>" .$row["Country"]. "</td></tr>";
        	}

        	echo "</table>";

		$insertArt = "INSERT INTO queryValidation (ValidatedQuery) VALUES('$searchVal')";
		mysqli_query($conn,$insertArt);
		$insertHit = "UPDATE queryValidation SET Hits = Hits + 1 WHERE ValidatedQuery = '$searchVal'";
